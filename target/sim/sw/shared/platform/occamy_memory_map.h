...
#define soc_ctrl_scratch_base \
    (TILE_0_CLUSTER_0_PERIPH_BASE_ADDR + STX_SOC_SCRATCH_0_REG_OFFSET)
...
#define cluster_hw_barrier_base              \
    (TILE_0_CLUSTER_0_PERIPH_BASE_ADDR + \
     SNITCH_CLUSTER_PERIPHERAL_HW_BARRIER_REG_OFFSET)

#define cluster_zero_memory_base TILE_0_CLUSTER_0_ZERO_MEM_BASE_ADDR
...
//    return translate_address(address, cluster_idx, cluster_offset);
    return translate_address(address, cluster_idx, 0);
...
inline uintptr_t cluster_tcdm_start_addr(uint32_t cluster_idx) {
    ...
    //                                  cluster_idx);
    return translate_cluster_address(TILE_0_CLUSTER_0_TCDM_BASE_ADDR,
                                     cluster_idx);
}
